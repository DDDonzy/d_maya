...
from UTILS.todo.get_softComponent import get_soft_selection_component
from UTILS.dag.getHistory import get_history


def autoProp(autoSkin=True):
    try:
        component_data = get_soft_selection_component()
    except Exception:
        cmds.error("Please select mesh or its components.")
        raise

    pos_mAry = []
    for mesh_name, component_weights_dict in component_data.items():
        mSel: om.MSelectionList = om.MSelectionList()
        mSel.add(mesh_name)
        fnMesh:om.MFnMesh = om.MFnMesh(mSel.getDagPath(0))
        pos_mAry += [fnMesh.getPoint(x, om.MSpace.kWorld) for x in component_weights_dict.keys()]
    ...
    jnt = cmds.createNode("joint", name="JNT_prop")
    cmds.xform(jnt, ws=1, matrix=center_matrix)

    if autoSkin:
        # auto skin
        for name, component_weights_dict in component_data.items():
            try:
                # try skin
                skin_node = cmds.skinCluster(name, jnt, tsb=1, rui=0, name=f"{name}_skinCluster", inf=jnt)[0]
            except Exception:
                # if skin exists, add influence
                skin_node = get_history(name, type="skinCluster")[0]
                if skin_node:
                    cmds.skinCluster(skin_node, e=1, ai=jnt, wt=0)

            # set weights
            selections_vertex_idx = list(component_weights_dict.keys())
            component_weights = list(component_weights_dict.values())
