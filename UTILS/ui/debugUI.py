...
class WidgetTreeModel(QStandardItemModel):
    def __init__(self, root_widget, parent=None):
        ...
        self.cppPtr = hex(shiboken2.getCppPointer(self.root_widget)[0])
    ...
    def populate_tree(self, parent_item=None, parent_widget=None):
        if parent_item is None:
            ...
            root_id_item = QStandardItem(self.cppPtr)
        ...
        for child in parent_widget.children():
            if isinstance(child, QWidget):
                ...
                id_item = QStandardItem(hex(shiboken2.getCppPointer(child)[0]))
...
class WidgetDebugTool(QMainWindow):
    ...
    def init_ui(self):
        ...
        # self.tree_view.header().resizeSection(1, 600)
        self.tree_view.header().resizeSection(1, 300)
        self.tree_view.header().resizeSection(2, 300)
        self.tree_view.header().resizeSection(3, 10)

        # 
        self.tree_view.header().moveSection(1,2)
        self.tree_view.header().moveSection(3,0)
...
def getMayaControl(name):
    ...
    if maya_control:
        return shiboken2.wrapInstance(int(maya_control), QWidget)
    return None
...
if __name__ == "__main__":
    ...
    cmds.ShapeEditor()
    cmds.refresh(f=1)
    bsPanelName = cmds.getPanel(type="shapePanel")[0]+"Window"
