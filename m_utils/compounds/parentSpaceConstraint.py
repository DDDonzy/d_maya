...
from m_utils.transform import get_localMatrix, get_offsetMatrix, get_worldMatrix
...
class parentSpaceConstraint(CreateBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(self.proxyAttrObject, str):
            self.proxyAttrObject = [self.proxyAttrObject]
        elif isinstance(self.proxyAttrObject, list):
            pass
        else:
            raise TypeError("proxyAttrObject must be a string or list of strings.")
    ...
    def create(self):
        ...
        for i, controller in enumerate(self.controller):
            self.addParentSpaceController(controller=controller, niceName=self.niceName[i])
    ...
    def createParentSpaceLogic(self):
        ...
        for obj in self.proxyAttrObject:
            if cmds.objExists(f"{obj}.{self.attrName}"):
                cmds.deleteAttr(f"{obj}.{self.attrName}")
            cmds.addAttr(obj, ln=self.attrName, k=1, pxy=self.parentspace)

    def addParentSpaceController(self, controller: str, niceName: str):
        # get index
        for x in self.proxyAttrObject:
            enum_str = cmds.addAttr(f"{x}.{self.attrName}", q=1, en=1)
            ...
            cmds.addAttr(f"{x}.{self.attrName}", e=1, en=enum_str)
            cmds.addAttr(self.parentspace, e=1, en=enum_str)

        # offset matrix to chose
        offsetMatrix = get_offsetMatrix(get_worldMatrix(self.target), get_worldMatrix(controller))
