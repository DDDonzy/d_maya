...
class CurveData(om.MFnNurbsCurve):
    ...
    def __init__(self, controlPoints, degree):
        om.MFnNurbsCurve.__init__(self)
    ...
    def build(self):
        self.create(self._controlPoints,
                    self._knots[1:-1],
                    self.degree,
                    self.form,
                    False,
    ...
    def get_tWeights(self, t):
        return [self.basisFunction(i, t, self.degree) for i in xrange(len(self.cvPositions()))]
    ...
    def generateKnots(self, controlPoints, degree):
        ...
        knots = [0.0] * d
        knots += [i / (count - d) for i in xrange(count - d + 1)]
        knots += [1.0] * d
        return knots
